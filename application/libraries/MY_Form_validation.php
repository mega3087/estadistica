		...
		*/
		...
		function valida_rfc($valor) {
			...
			//RFC Persona Física.
				...
				$this->form_validation->set_message('valida_rfc', 'El campo %s tiene un formato invalido, Ejemplo: XXXXDDDDDDXXX');
				return false;
			}
		}
		
		function valida_numero($value='',$rango_numeros=''){
			if (!isset($value))
				return true;
			$CI =& get_instance();
			$pattern = "/^[0-9]{1,3}(?:,?[0-9]{3})*(?:\.[0-9]{2})?$/";
			if(!preg_match($pattern,$value)){
				$CI->form_validation->set_message('valida_numero', 'El campo %s es inválido, use el formato: ###');
				return false;
			}else{					
				if($rango_numeros) {
					list($rango_inicial,$rango_final) = explode(',', $rango_numeros);

					if ($rango_inicial) {
						if ($value < $rango_inicial) {
							$CI->form_validation->set_message('valida_numero', 'El campo %s debe ser igual o mayor a '.$rango_inicial );
							return false;
						}
					}	
					
					if ($rango_final) {
						if ($value > $rango_final) {
							$CI->form_validation->set_message('valida_numero', 'El campo %s debe ser igual o menor a '.$rango_final );
							return false;
						}
					}
				}
