<?php
	...
	{	
		...
		function index()
		{
			$this->db->join('vecaccion', 'CPEAccion=CACClave','RIGHT');
			$this->db->join('vecmodulo', 'CACModulo=CMOClave','RIGHT');
			$this->db->group_by('cpeaccion');
			$this->db->order_by('CMODescripcion,CACDescripcion');
		...
		function update(){
			...
			$this->db->empty_table('vecpermiso');
