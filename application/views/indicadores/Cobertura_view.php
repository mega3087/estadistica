					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover dataTables-example- dataTable" >
							...
							<tbody>
								...
								<tr>
									<td><?=$list['APeriodo']?></td>
									<td><?=$list['ICobertura']?> % </td>
									<td>
									<button data-toggle="modal" href="#modal_cobertura" 
										data-iidcobertura_skip="<?php echo $list['Iidcobertura']; ?>"
										data-aperiodo="<?php echo $list['APeriodo']; ?>"
										data-icobertura="<?php echo $list['ICobertura']; ?>"								
...
<div class="modal inmodal" id="modal_cobertura" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog" >
		<div class="modal-content animated flipInY">
			
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Cerrar</span></button>
				<h6 class="modal-title"><i class="fa fa-building-o"></i>&nbsp;&nbsp;Cobertura</h6>
			</div>
			<div class="modal-content">
				<?php echo form_open('indicadores/save_cobertura', array('name' => 'Formcobertura', 'id' => 'Formcobertura', 'role' => 'form', 'class' => 'form-horizontal panel-body')); ?>
				<div class="form-group">
				...
				<div class="form-group">
					<label class="col-lg-3 control-label" for="">COBERTURA: <em>*</em></label>
					<div class="col-lg-9">
						<input type="text" id="ICobertura" name="ICobertura" value="" maxlength='150' class="form-control uppercase"/>
					</div>
				</div>
				...
				<div class="form-group">
					<div class="col-lg-offset-3 col-lg-9">
						<?php if( is_permitido(null,'indicadores','save_cobertura') ){ ?>
						<input type="hidden" id="Iidcobertura_skip" name="Iidcobertura"/>
						<button type="submit" class="btn btn-primary pull-right"> <i class="fa fa-save"></i> Guardar</button>
						<?php } ?>
					</div>
				</div>
...
<div class="modal inmodal" id="modal_agregar" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog" >
		<div class="modal-content animated flipInY">
			...
			<div class="modal-content">
				<?php echo form_open('indicadores/agregar_cobertura', array('name' => 'Formagregar_cobertura', 'id' => 'Formagregar_cobertura', 'role' => 'form', 'class' => 'form-horizontal panel-body')); ?>
				<div class="form-group">
					<label class="col-lg-3 control-label" for="">CICLO:<em>*</em></label>
					<div class="col-lg-9">
						<input type="text" id="APeriodo" name="APeriodo" value="" maxlength='150' class="form-control uppercase"/>
				...
				<div class="form-group">
					<div class="col-lg-offset-3 col-lg-9">
						<?php if( is_permitido(null,'indicadores','agregar_cobertura') ){ ?>
							<button type="submit" class="btn btn-primary pull-right"> <i class="fa fa-save"></i> Guardar</button>
						<?php } ?>
					</div>
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(document).on("click", ".open", function () {
			$(".modal-content #Iidcobertura_skip").val( $(this).data('iidcobertura_skip') );
			$(".modal-content #APeriodo").val( $(this).data('aperiodo') );
			$(".modal-content #ICobertura").val( $(this).data('icobertura') );
		});

		$("#Formcobertura").submit(function(e) {
			e.preventDefault();
			$.ajax({
				type: "POST",
				url: "<?php echo base_url("indicadores/save_cobertura"); ?>",
				data: $(this).serialize(),
				dataType: "html",
				beforeSend: function(){
		...
		$("#Formagregar_cobertura").submit(function(e) {
			e.preventDefault();
			$.ajax({
				type: "POST",
				url: "<?php echo base_url("indicadores/agregar_cobertura"); ?>",
				data: $(this).serialize(),
				dataType: "html",
				beforeSend: function(){
					$(".loading").html("<div class=\"spiner-example\"><div class=\"sk-spinner sk-spinner-three-bounce\"><div class=\"sk-bounce1\"></div><div class=\"sk-bounce2\"></div><div class=\"sk-bounce3\"></div></div></div>");
