				...
				<div class="ibox-content">
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover dataTables-example- dataTable" >
							...
							<tbody>
								<?php foreach($oferta as $key => $list_o ){ ?>
						<tr>
							
							<td><?=$list_o['OEPeriodo']?></td>
							<td><?=$list_o['OETotal']?></td>
							
							<td><button data-toggle="modal" href="#modal_oferta" 
								data-oeidoferta_skip="<?php echo $list_o['OEIdOferta']; ?>"
								data-oeperiodo="<?php echo $list_o['OEPeriodo']; ?>"
								data-oetotal="<?php echo $list_o['OETotal']; ?>"
			...
			<div class="col-lg-8">
				<script type="text/javascript">
					$(function () {
						var barData = {
							labels: [
								<?php foreach($oferta as $key => $list ){ ?>
									"<?=$list['OEPeriodo']?>",
								<?php } ?>
							],
							datasets: [
								{
									...
									fillColor: "rgba(113,127,86,0.5)",
									strokeColor: "rgba(113,127,86,0.8)",
									highlightFill: "rgba(113,127,86,0.75)",
									highlightStroke: "rgba(113,127,86,1)",
									data: [
										<?php foreach($oferta as $key => $list ){ ?>
											"<?=$list['OETotal']?>",
										<?php } ?>
...
<div class="modal inmodal" id="modal_oferta" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog" >
		<div class="modal-content animated flipInY">
			
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Cerrar</span></button>
				<h6 class="modal-title"><i class="fa fa-building-o"></i>&nbsp;&nbsp; Oferta Educativa</h6>
			</div>
			<div class="modal-content">
				<?php echo form_open('indicadores/save_oferta', array('name' => 'Formofertaeducativa', 'id' => 'Formofertaeducativa', 'role' => 'form', 'class' => 'form-horizontal panel-body')); ?>
				<div class="form-group">
					<label class="col-lg-3 control-label" for="">CICLO:<em>*</em></label>
					<div class="col-lg-9">
						<input type="text" id="OEPeriodo" name="OEPeriodo" value="" maxlength='150' class="form-control uppercase" />
					</div>
				</div>
				<div class="form-group">
					<label class="col-lg-3 control-label" for="">TOTAL DE otal Oferta Educativa: <em>*</em></label>
					<div class="col-lg-9">
						<input type="text" id="OETotal" name="OETotal" value="" maxlength='150' class="form-control uppercase" />
					</div>
				</div>
				...
				<div class="form-group">
					<div class="col-lg-offset-3 col-lg-9">
						...
						<input type="hidden" id="OEidOferta_skip" name="OEIdOferta" />
						<button type="submit" class="btn btn-primary pull-right"> <i class="fa fa-save"></i> Guardar</button>
						<!--<?php //} ?>-->
					</div>
				</div>
...
<div class="modal inmodal" id="modal_agregar" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog" >
		<div class="modal-content animated flipInY">
			...
			<div class="modal-content">
				<?php echo form_open('indicadores/agregar_oferta', array('name' => 'Formagregar_oferta', 'id' => 'Formagregar_oferta', 'role' => 'form', 'class' => 'form-horizontal panel-body')); ?>
				<div class="form-group">
					<label class="col-lg-3 control-label" for="">Período:<em>*</em></label>
					<div class="col-lg-9">
						<input type="text" id="OEPeriodoA" name="OEPeriodo" value="" maxlength='150' class="form-control uppercase"/>
					</div>
				</div>				
				
				<div class="form-group">
					<label class="col-lg-3 control-label" for="">Oferta Educativa: <em>*</em></label>
					<div class="col-lg-9">
						<input type="text" id="OETotalA" name="OETotal" value="" maxlength='150' class="form-control uppercase"/>
					</div>
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(document).on("click", ".open", function () {
			$(".modal-content #OEidOferta_skip").val( $(this).data('oeidoferta_skip') );
			$(".modal-content #OEPeriodo").val( $(this).data('oeperiodo') );
			$(".modal-content #OETotal").val( $(this).data('oetotal') );
		});

		$("#Formofertaeducativa").submit(function(e) {
			e.preventDefault();
			$.ajax({
				type: "POST",
				url: "<?php echo base_url("indicadores/save_oferta"); ?>",
				data: $(this).serialize(),
				dataType: "html",
				beforeSend: function(){
		...
		$("#Formagregar_oferta").submit(function(e) {
					e.preventDefault();
					$.ajax({
						type: "POST",
						url: "<?php echo base_url("indicadores/agregar_oferta"); ?>",
						data: $(this).serialize(),
						dataType: "html",
						beforeSend: function(){
							$(".loading").html("<div class=\"spiner-example\"><div class=\"sk-spinner sk-spinner-three-bounce\"><div class=\"sk-bounce1\"></div><div class=\"sk-bounce2\"></div><div class=\"sk-bounce3\"></div></div></div>");
